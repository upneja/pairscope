...
import { transformClaudeResponse } from "@/lib/report-transformer";
import {
  placeholderReportRelationship,
  placeholderReportSingle,
} from "@/lib/placeholder-report";

const SYSTEM_PROMPT = `You are the AI synthesis engine for **Pairscope**, a research-backed relationship health diagnostic. Your job is to transform structured assessment scores into a personalized, narrative Relationship Health Report delivered as strict JSON.
...
export async function POST(request: NextRequest) {
  try {
    const body: AssessmentSubmissionPayload = await request.json();

    // Validate basic structure
    if (!body.mode || !body.scores) {
      return NextResponse.json(
        { error: "Missing required fields: mode and scores" },
        { status: 400 }
      );
    }
    ...
    if (!apiKey) {
      console.warn(
        "ANTHROPIC_API_KEY not set — returning placeholder report for demo"
      );
    ...
    // Build Claude input: map frontend mode to Agent 2 mode
    ...
    let claudeResponse;
    try {
      claudeResponse = JSON.parse(textBlock.text);
    } catch (parseError) {
      console.error("Failed to parse Claude response:", textBlock.text.substring(0, 500));
      throw new Error(`Claude returned invalid JSON: ${parseError}`);
    }

    // Transform to frontend Report type
    const report = transformClaudeResponse(claudeResponse);

    return NextResponse.json(report);
  } catch (error) {
    console.error("Report generation failed:", error);

    // Return placeholder as fallback on error
    const fallbackReport = placeholderReportRelationship;
    return NextResponse.json(
      {
        ...fallbackReport,
        _error: "Report generation encountered an issue. Showing sample report.",
        _errorDetail:
          error instanceof Error ? error.message : "Unknown error",
      },
      { status: 200 } // Return 200 with fallback so frontend still renders
    );
  }
}
